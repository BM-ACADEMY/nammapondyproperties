...
const Testimonials = () => {
  ...
  useEffect(() => {
    const fetchTestimonials = async () => {
      try {
        ...
        console.log("Testimonials data:", res.data); // Debug log
  ...
  const handleCreateReview = async (values) => {
    if (!isAuthenticated)
      return message.error("Please login to write a review");

    setSubmitLoading(true);
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/testimonials`, {
        ...values,
        user_id: user?.user?._id || user?._id, // Handle auth structure quirk safely
        name: user?.user?.name || user?.name || "User",
        role: "User", // Default role since we removed the input
      });
