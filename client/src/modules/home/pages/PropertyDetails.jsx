...
const PropertyDetails = () => {
    ...
    useEffect(() => {
        const fetchProperty = async () => {
            try {
                const res = await axios.get(`${import.meta.env.VITE_API_URL}/properties/fetch-property-by-id/${id}`);
                ...
                const relatedRes = await axios.get(`${import.meta.env.VITE_API_URL}/properties/fetch-all-property?limit=4`);
                if (Array.isArray(relatedRes.data.properties)) {
                    setMoreProperties(relatedRes.data.properties.filter(p => p._id !== id));
                }
            } catch (error) {
    ...
    const submitEnquiry = async (name, email, phone) => {
        ...
        const message = `Hi, I am interested in your property: ${property.title} located at ${property.location}. Please provide more details.`;
    ...
    const getImageUrl = (path) => path ? `${import.meta.env.VITE_API_URL.replace('/api', '')}${path}` : 'https://placehold.co/800x600';
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                <div className="p-3 bg-gray-50 rounded-lg">
                                    ...
                                    <div className="font-semibold text-gray-800 flex items-center">
                                        <div className="w-4 h-4 mr-2 rounded-full border border-blue-500 flex items-center justify-center text-[10px] text-blue-500 font-bold">✓</div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-md border border-gray-100 sticky top-24">
                            <div className="flex items-center gap-4 mb-6">
                                <div className="w-16 h-16 bg-gray-200 rounded-full overflow-hidden">
                                    {property.seller_id?.profile_image ? (
                                        <img src={getImageUrl(property.seller_id.profile_image)} alt="Seller" className="w-full h-full object-cover" />
                                    ) : (
                                ...
                                <div>
                                    <div className="text-xs text-gray-500 uppercase font-semibold tracking-wide">Developer / Seller</div>
                                    <h3 className="text-lg font-bold text-gray-900">{property.seller_id?.name || "Verified Seller"}</h3>
                            ...
                            <p className="text-sm text-gray-500 mb-6 text-center">
                                Contact us to get more details, brochure, or schedule a site visit.
                            </p>
                    ...
                    <div className="mt-16">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {moreProperties.map((prop) => (
                                <Link to={`/properties/${prop._id}`} key={prop._id} className="block group">
                                    <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition">
                                        ...
                                        <div className="p-4">
                                            ...
                                            <p className="text-blue-600 font-bold mt-1">₹ {prop.price.toLocaleString()}</p>
                                        </div>
                                    </div>
                                </Link>
                            ))}
