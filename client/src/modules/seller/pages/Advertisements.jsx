...
import { Table, Tag, Button, message, Typography, Card, Badge, Space } from "antd";
import { Plus, Megaphone, Clock, CheckCircle, ExternalLink } from "lucide-react";
...
const SellerAdvertisements = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            key: "status",
            render: (_, record) => {
                const request = marketingRequests.find(r => r.property_id._id === record._id);
        ...
        },
        {
            ...
            render: (_, record) => {
                const hasActiveRequest = marketingRequests.some(
                    r => r.property_id._id === record._id && ["pending", "contacted"].includes(r.status)
                );
                ...
                return (
                    <Button
                        ...
                        icon={hasActiveRequest ? <CheckCircle size={14} /> : <Megaphone size={14} />}
                        onClick={() => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <Card className="bg-gradient-to-br from-indigo-600 to-purple-700 text-white border-none shadow-lg">
                    <div className="flex justify-between items-start">
                        <div>
                            <div className="text-sm font-semibold uppercase tracking-wider">Pending Requests</div>
                            <div className="text-4xl font-black mt-2 drop-shadow-md">
                                {marketingRequests.filter(r => r.status === 'pending').length}
                            </div>
                        </div>
                        <div className="bg-white/10 p-3 rounded-xl backdrop-blur-sm border border-white/10"><Clock size={28} /></div>
                    </div>
                </Card>
                <Card className="bg-gradient-to-br from-emerald-600 to-teal-700 text-white border-none shadow-lg">
                    <div className="flex justify-between items-start">
                        <div>
                            <div className="text-sm font-semibold uppercase tracking-wider">Active Promotions</div>
                            <div className="text-4xl font-black mt-2 drop-shadow-md">
                                {marketingRequests.filter(r => r.status === 'completed').length}
                            </div>
                        </div>
                        <div className="bg-white/10 p-3 rounded-xl backdrop-blur-sm border border-white/10"><CheckCircle size={28} /></div>
                    </div>
                </Card>
            </div>
